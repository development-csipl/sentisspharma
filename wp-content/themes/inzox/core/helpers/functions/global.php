...
// ----------------------------------------------------------------------------------------
function inzox_option( $key, $default_value = '', $method = 'customizer' ) {
	if ( defined( 'FW' ) ) {
		switch ( $method ) {
			case 'theme-settings':
...
// ----------------------------------------------------------------------------------------
function inzox_meta_option( $postid, $key, $default_value = '' ) {
	if ( defined( 'FW' ) ) {
...
// ----------------------------------------------------------------------------------------
function inzox_resize( $url, $width = false, $height = false, $crop = false ) {
...
// ----------------------------------------------------------------------------------------
function inzox_src( $key, $default_value = '', $input_as_attachment = false ) { // for src
	if ( $input_as_attachment == true ) {
		$attachment = $key;
	} else {
		$attachment = inzox_option( $key );
	}
...
// ----------------------------------------------------------------------------------------
function inzox_org_id( $id, $name = true ) {
...
// ----------------------------------------------------------------------------------------
function inzox_google_fonts_url($font_families	 = []) {
	$fonts_url		 = '';
	if ( $font_families ) {
		$query_args = array(
...
// ----------------------------------------------------------------------------------------
function inzox_get_mega_item_child_slug( $location, $option_id ) {
	$mega_item	 = '';
...
// ----------------------------------------------------------------------------------------
function inzox_youtube_cover( $e ) {
	$src = null;
	//get the url
	if ( $e != '' ){
		$url = $e;
...
// ----------------------------------------------------------------------------------------
function inzox_soundcloud_embed( $url ) {
...
// ----------------------------------------------------------------------------------------
function inzox_video_embed($url){
...
if ( !function_exists( 'inzox_advanced_font_styles' ) ) :

	/**
	 * Get shortcode advanced Font styles
	 *
	 */
	function inzox_advanced_font_styles( $style ) {
...
function inzox_array_elements_remove( $input ,$elements){
...
// review rating
function inzox_review_rating($arg = []){
